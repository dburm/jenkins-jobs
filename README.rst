Automated jenkins jobs and views configuration CI
...
``conf/jenkins_job.ini.example``
  ...
  Example config file. Can be used as is to run tests, when tests
...
``servers/<jenkins-master-id>/``
  ...
  folder. For example, ``servers/infra-ci/`` folder corresponds to
  infra-ci.yourdomain.com.
